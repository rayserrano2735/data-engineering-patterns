...
echo ===============================================
echo Python Analytics Interview Prep v0.6.3
echo ===============================================
echo.
...
set SCRIPT_DIR=%~dp0
set ZIP_FILE=%SCRIPT_DIR%paip-platform-v0.6.3.zip

if not exist "%ZIP_FILE%" (
    echo ERROR: Cannot find paip-platform-v0.6.3.zip
...
if errorlevel 1 (
    ...
    git add README.md
    git add study/
    git commit -m "Platform v0.6.3"
    
    REM Create tag
    git tag -a v0.6.3 -m "Release v0.6.3"
    echo Created tag v0.6.3
) else (
    ...
    REM Commit
    git commit -m "Platform v0.6.3" >nul 2>&1
    if errorlevel 1 (
        echo No changes to commit
    ) else (
        echo Committed platform v0.6.3
    )
    
    REM Tag (silently fail if exists)
    git tag -a v0.6.3 -m "Release v0.6.3" 2>nul
    if errorlevel 1 (
        echo Note: Tag v0.6.3 already exists
    ) else (
        echo Created tag v0.6.3
    )
)
