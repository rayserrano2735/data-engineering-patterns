...
echo ===============================================
echo Python Analytics Interview Prep v1.0.1
echo ===============================================
echo.
...
set SCRIPT_DIR=%~dp0
set ZIP_FILE=%SCRIPT_DIR%paip-platform-v1.0.1.zip

if not exist "%ZIP_FILE%" (
    echo ERROR: Cannot find paip-platform-v1.0.1.zip
...
REM Clean study folder of zip artifacts
echo Cleaning study workspace...
for /d %%D in (study\*) do (
    if /i not "%%D"=="study\practice_work" (
        if /i not "%%D"=="study\notes" (
            if /i not "%%D"=="study\mock_interviews" (
                echo Removing artifact: %%D
                rd /s /q "%%D" >nul 2>&1
            )
        )
    )
...
REM Clean platform/content folder of zip artifacts
...
if errorlevel 1 (
    ...
    git add README.md
    git add study/
    git commit -m "Platform v1.0.1"
    
    REM Create tag
    git tag -a v1.0.1 -m "Release v1.0.1"
    echo Created tag v1.0.1
) else (
    ...
    REM Commit
    git commit -m "Platform v1.0.1" >nul 2>&1
    if errorlevel 1 (
        echo No changes to commit
    ) else (
        echo Committed platform v1.0.1
    )
    
    REM Tag (silently fail if exists)
    git tag -a v1.0.1 -m "Release v1.0.1" 2>nul
    if errorlevel 1 (
        echo Note: Tag v1.0.1 already exists
    ) else (
        echo Created tag v1.0.1
    )
)
...
if /i not "%COPY_TO_VC%"=="n" (
    ...
    copy "%SCRIPT_DIR%paip-install-v0.6.3.bat" "!VC_DIR!\" >nul
