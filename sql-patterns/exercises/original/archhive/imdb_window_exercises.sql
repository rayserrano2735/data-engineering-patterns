...
SELECT 
    PRIMARY_TITLE,
    RUNTIME_MINUTES,
    -- Add NTILE(4) here to create quartiles
FROM title_basics
WHERE TITLE_TYPE = 'movie'
    AND RUNTIME_MINUTES IS NOT NULL
    AND START_YEAR = 2023
...
WITH ranked_runtimes AS (
    -- Your code here using DENSE_RANK
)
SELECT * FROM ranked_runtimes WHERE -- Your filter for second highest;
...
SELECT 
    START_YEAR,
    -- Add PERCENTILE_CONT for median
FROM title_basics
WHERE TITLE_TYPE = 'movie'
    AND RUNTIME_MINUTES IS NOT NULL
...
-- TRAP 1: "Find duplicates" - They want GROUP BY, not window functions!
SELECT 
    TITLE_CODE,
...
-- TRAP 2: "Remove duplicates" - Now they want window functions!
WITH deduped AS (
    SELECT *, ROW_NUMBER() OVER (PARTITION BY TITLE_CODE ORDER BY INGESTION_TS_UTC DESC) as rn
    FROM title_basics
)
SELECT * FROM deduped WHERE rn = 1;

-- TRAP 3: They say "average" but mean "median"
-- Always clarify: "Do you want the mean or median?"