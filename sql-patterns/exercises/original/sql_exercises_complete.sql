...
SELECT * FROM ranked_movies WHERE rn <= 10;
...
SELECT 
    ...
    SUM(COUNT(*)) OVER (
        ORDER BY START_YEAR
        ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW  -- Correct direction!
    ) as running_total
FROM title_basics
WHERE TITLE_TYPE = 'movie'
GROUP BY START_YEAR
...
SELECT 
    ...
    GENRES,
    RUNTIME_MINUTES,
    RANK() OVER (PARTITION BY GENRES ORDER BY RUNTIME_MINUTES DESC) as runtime_rank_within_genre
FROM title_basics
WHERE TITLE_TYPE = 'movie' 
    AND GENRES IS NOT NULL 
    AND RUNTIME_MINUTES IS NOT NULL
